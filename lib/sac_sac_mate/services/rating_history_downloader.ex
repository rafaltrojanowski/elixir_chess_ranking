defmodule SacSacMate.Services.RatingHistoryDownloader do
  ...
  @versions %{txt: "", xml: "_xml"}
  @extension ".zip"

  @file_path "files/"

  @doc "Fow now just grab TXT"
  def call() do
    Enum.each years_range, fn year ->
      ...
      Enum.each @months, fn month ->
        Enum.each @categories, fn category ->
          link = "#{@base_url}#{category}_#{month}#{year}frl#{@extension}"
          download(link)
        end
      end
    end
  ...
  defp years_range do
    end_year = date = Date.utc_today().year
